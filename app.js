...
import { initializeRecommendPage } from './recommend.js'; // ✅ 從 recommend.js 匯入函數
...
export const APP_URLS = { // ✅ 匯出，讓 recommend.js 也能使用
...
function renderNavTabs() { /* ... 您原本的 renderNavTabs 函數 ... */ }
async function loadMemberName(email) { /* ... 您原本的 loadMemberName 函數 ... */ }
async function loadExternalHtmlSection(sectionId) { /* ... 您原本的 loadExternalHtmlSection 函數 ... */ }

function navigateTo(id, fromHistory = false) {
    pages.forEach(p => p.style.display = 'none');
    if (id === 'recommend') {
        recommendPage.style.display = 'block';
        initializeRecommendPage(); // ✅ 呼叫從 recommend.js 來的函數
    } else {
...
window.navigateTo = navigateTo; // 讓 HTML onclick 可以找到

// --- 事件監聽與啟動邏輯 ---
document.body.addEventListener("click", function (e) { /* ... 您原本的 click 函數 ... */ });
window.addEventListener('popstate', function(event) { /* ... 您原本的 popstate 函數 ... */ });
onAuthStateChanged(auth, (user) => { /* ... 您原本的 onAuthStateChanged 函數 ... */ });
...
document.body.addEventListener("click",function(e){const clickedLink=e.target.closest("a[data-section]");if(clickedLink){e.preventDefault();const id=clickedLink.dataset.section;if(id==="logout"){signOut(auth).catch((error)=>console.error("登出失敗:",error))}else{navigateTo(id)}}});
